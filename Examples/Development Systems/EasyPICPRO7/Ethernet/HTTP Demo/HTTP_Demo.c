 ...
 * NOTES:
     ...
     - To simulate analog inputs on ADC channel 2 and 6, use on-board potentiometer P2 and P3,
       and place jumper J5 to MCU pin RA2 and jumper J6 to MCU pin RF1. (board specific)
 */
...
<tr><td>AN6</td><td><script>document.write(AN6)</script></td></tr>\
...
unsigned int    Ethernet_UserTCP(unsigned char *remoteHost, unsigned int remotePort, unsigned int localPort, unsigned int reqLength, TEthPktFlags *flags)
        ...
        if(getRequest[5] == 's')                    // if request path name starts with s, store dynamic data in transmit buffer
                ...
                // add AN6 value to reply
                IntToStr(ADC_Read(6), dyna) ;
                len += putConstString("var AN6=") ;
                len += putString(dyna) ;
                len += putConstString(";") ;
...
void    main()
        {
        ADCON1 = 0x08 ;         // ADC convertors will be used with AN0 and AN6
