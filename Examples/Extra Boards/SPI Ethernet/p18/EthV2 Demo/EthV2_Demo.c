 ...
 * Target Platform:
     PIC18F with more than 32 Kb ROM
 * Copyright:
 ...
 *      the INT and WOL signals from the ENC are not used *      the INT and WOL signals from the ENC are not used
 *
 * Test configuration:
     MCU:             PIC18F4525
...
void    interrupt()
        ...
        if(INTCON.TMR0IF)                        // timer0 overflow ?
                ...
                presTmr++ ;                      // increment prescaler
                            ...
                            if(presTmr == 15625)             // timer0 overflows 15625 times per second
                        {
                        SPI_Ethernet_UserTimerSec++ ;  // increment ethernet library counter
                        epoch++ ;                      // epoch is RTC counter
                        presTmr = 0 ;                  // reset prescaler
                        }
                if(lcdTmr == 3125 )              // it takes lcdTmr 1562.5 increments for
                  {                              // a 100ms delay at 32MHz with current timer0 settings
                                                 // so for 200ms we need 3125 increments
